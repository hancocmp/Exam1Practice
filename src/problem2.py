...
def problem2b(rect, n, delta, win):
    ...
    for k in range(n-1):
        new_corner1 = rg.Point(corner_1.x - ((k+1)*delta), corner_1.y - ((k+1)*delta))
        new_corner2 = rg.Point(corner_2.x + ((k+1)*delta), corner_2.y + ((k+1)*delta))
        rect1 = rg.Rectangle(new_corner1, new_corner2)
        rect1.attach_to(win)
        print(new_corner1, new_corner2)
    win.render()
